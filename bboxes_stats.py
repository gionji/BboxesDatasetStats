...
# Print class statistics
    for class_label, stats in class_statistics.items():
        ...
        print(f"  Regular Bboxes - Average Size: {stats['regular']['total_width'] / stats['regular']['bbox_count'] if stats['regular']['bbox_count'] > 0 else 0} (width) x {stats['regular']['total_height'] / stats['regular']['bbox_count'] if stats['regular']['bbox_count'] > 0 else 0} (height)")
        print(f"  Regular Bboxes - Average Area: {stats['regular']['total_area'] / stats['regular']['bbox_count'] if stats['regular']['bbox_count'] > 0 else 0}")
        print(f"  Regular Bboxes - Median Area: {statistics.median(stats['regular']['areas']) if stats['regular']['bbox_count'] > 0 else 0}")
        print(f"  Regular Bboxes - Number of Bounding Boxes: {stats['regular']['bbox_count']}")
        print("")
        print(f"  Removed Bboxes - Average Size: {stats['removed']['total_width'] / stats['removed']['bbox_count'] if stats['removed']['bbox_count'] > 0 else 0} (width) x {stats['removed']['total_height'] / stats['removed']['bbox_count'] if stats['removed']['bbox_count'] > 0 else 0} (height)")
        print(f"  Removed Bboxes - Average Area: {stats['removed']['total_area'] / stats['removed']['bbox_count'] if stats['removed']['bbox_count'] > 0 else 0}")
        print(f"  Removed Bboxes - Median Area: {statistics.median(stats['removed']['areas']) if stats['removed']['bbox_count'] > 0 else 0}")
        print(f"  Removed Bboxes - Number of Bounding Boxes: {stats['removed']['bbox_count']}")
        print("")
        print(f"  Overall - Average Size: {stats['overall']['total_width'] / stats['overall']['bbox_count'] if stats['overall']['bbox_count'] > 0 else 0} (width) x {stats['overall']['total_height'] / stats['overall']['bbox_count'] if stats['overall']['bbox_count'] > 0 else 0} (height)")
        print(f"  Overall - Average Area: {stats['overall']['total_area'] / stats['overall']['bbox_count'] if stats['overall']['bbox_count'] > 0 else 0}")
        print(f"  Overall - Median Area: {statistics.median(stats['overall']['areas']) if stats['overall']['bbox_count'] > 0 else 0}")
        print(f"  Overall - Number of Bounding Boxes: {stats['overall']['bbox_count']}")
        print("")



def plot_bbox_area_distribution(json_data_list, vehicle_class=None, bbox_type='all', n_bins=40):
    areas = []
    ...
    for json_data in json_data_list:
        ...
        if vehicle_class is not None:
            bboxes = [bbox for bbox, v_class in zip(bboxes, json_data.get('vehicle_class', [])) if v_class == vehicle_class]
    ...
    title = f'Distribution of Bounding Box Areas - Class {vehicle_class if vehicle_class is not None else "All"} - Type {bbox_type.capitalize()}'
    plt.title(title)
    ...
    save_plot_as_png(plt, f"bbox_area_distribution_class_{vehicle_class}_type_{bbox_type}")




def plot_metadata_distribution(json_data_list):
    ...
    #plt.show()
    # Save the plot as PNG
    save_plot_as_png(plt, "metadata_distribution")



def plot_bbox_positions(json_data_list, vehicle_class=None, bbox_type='all'):
    ...
    title = f'Bounding Box Positions (CDF) - Class {vehicle_class if vehicle_class is not None else "All"} - Type {bbox_type.capitalize()}'
    plt.title(title)
    ...
    #plt.show()
    # Save the plot as PNG
    save_plot_as_png(plt, f"bbox_positions_class_{vehicle_class}_type_{bbox_type}")
...
def plot_bbox_areas(json_data_list, bbox_type='all', vehicle_class=None):
    ...
    for json_data in json_data_list:
        if bbox_type == 'all':
            if vehicle_class is not None:
                bboxes = [bbox for bbox, v_class in zip(json_data.get('bboxes', []), json_data.get('vehicle_class', [])) if v_class == vehicle_class]
            else:
                bboxes = json_data.get('bboxes', [])
        elif bbox_type == 'removed':
            if vehicle_class is not None:
                bboxes = [bbox for bbox, v_class in zip(json_data.get('removed_bboxes', []), json_data.get('removed_vehicle_class', [])) if v_class == vehicle_class]
            else:
                bboxes = json_data.get('removed_bboxes', [])
        elif bbox_type == 'regular':
            if vehicle_class is not None:
    ...
    title = f'Bounding Box Areas ({bbox_type.capitalize()} - Class {vehicle_class if vehicle_class is not None else "All"})'
    plt.title(title)
    plt.xlabel('X-coordinate')
    plt.ylabel('Y-coordinate')
    plt.colorbar()
    #plt.show()
    # Save the plot as PNG
    save_plot_as_png(plt, f"bbox_areas_class_{vehicle_class}_type_{bbox_type}")
